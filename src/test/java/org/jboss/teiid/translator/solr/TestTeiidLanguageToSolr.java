...
/*	private QueryMetadataInterface solrMetadata() {

		MetadataFactory factory = new MetadataFactory("", 1, "", SystemMetadata
				.getInstance().getRuntimeTypeMap(), new Properties(), "");
		createFakeMetadata(factory);
		return new TransformationMetadata(null, new CompositeMetadataStore(
				factory.asMetadataStore()), null,
				RealMetadataFactory.SFM.getSystemFunctions(), null);
	}*/

	/*private void createFakeMetadata(MetadataFactory factory) {
		
		 * Table person = factory.addTable("Person"); Column id =
		 * factory.addColumn("id", TypeFacility.RUNTIME_NAMES.INTEGER, person);
		 * Column name = factory.addColumn("name",
		 * TypeFacility.RUNTIME_NAMES.STRING, person);
		 Column age = factory.addColumn("age",
				TypeFacility.RUNTIME_NAMES.INTEGER, person);
	}*/
